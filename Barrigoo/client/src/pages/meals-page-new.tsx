          ...
          <ul className="text-sm text-gray-600 list-disc pl-5">
            {meal.ingredients.map((ingredient, i) => (
              <li key={i}>
                ...
                {ingredient.cookingMethod && ` - ${typeof ingredient.cookingMethod === 'string' ? ingredient.cookingMethod : ingredient.cookingMethod?.name}`} 
  ...
  const renderCookingMethodOptions = () => {
    if (Array.isArray(cookingMethods) && cookingMethods.length > 0) {
      if (typeof cookingMethods[0] === 'string') {
        // Safe cast to string array
        const stringMethods = cookingMethods as unknown as string[];
        return stringMethods.map((method) => (
          <SelectItem key={method} value={method}>
            {method}
          </SelectItem>
        ));
      } else {
        // Safe cast to CookingMethod array
        const objectMethods = cookingMethods as unknown as CookingMethod[];
      ...
      // Default cooking methods if none are available
      return ["Raw", "Boiled", "Steamed", "Fried", "Grilled", "Baked", "Roasted", "Sautéed"].map((method) => (
        <SelectItem key={method} value={method}>
          {method}
        </SelectItem>
      ));
  ...
  const handleCookingMethodChange = async (cookingMethodName: string, index: number) => {
    ...
    if (Array.isArray(cookingMethods) && cookingMethods.length > 0) {
      if (typeof cookingMethods[0] === 'string') {
        ...
        const method = (cookingMethods as CookingMethod[]).find(m => m.name === cookingMethodName);
        if (method) {
          ...
          ? selectedPremadeMeal.ingredients.map(ing => {
              ...
              return {
                ...
                cookingMethod: ing.cookingMethodId 
                  ? Array.isArray(cookingMethods) && cookingMethods.length > 0 && typeof cookingMethods[0] !== 'string'
                    ? (cookingMethods as CookingMethod[]).find(m => m.id === ing.cookingMethodId)?.name
                    : "Unknown"
                  : undefined
              };
            })
                      ...
                      {selectedPremadeMeal.ingredients.map((ing, idx) => {
                        ...
                        return (
                          <li key={idx} className="flex items-center">
                            ...
                            {cookingMethod && <span className="text-xs text-muted-foreground ml-1">- {typeof cookingMethod === 'string' ? cookingMethod : cookingMethod?.name}</span>}
                          </li>
                        );
                      })}
                    </ul>
